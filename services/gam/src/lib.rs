...
impl FocusState {
    pub fn convert_focus_change(code: usize) -> FocusState {
        if code != 0 { FocusState::Foreground } else { FocusState::Background }
    }
}

#[derive(Debug)]
pub struct Gam {
    /// The Gam structure exists on the client-side. This is the connection ID to the GAM server, local to
    /// this client.
    conn: xous::CID,
...
impl Gam {
    pub fn new(xns: &xous_names::XousNames) -> Result<Self, xous::Error> {
        ...
        Ok(Gam { conn, callback_sid: None })
    }

    pub fn conn(&self) -> CID { self.conn }

    pub fn getop_revert_focus(&self) -> u32 {
        // non-blocking version is handed out to the menu handler
        Opcode::RevertFocusNb.to_u32().unwrap()
    }

    pub fn redraw(&self) -> Result<(), xous::Error> {
        send_message(self.conn, Message::new_scalar(Opcode::Redraw.to_usize().unwrap(), 0, 0, 0, 0))
            .map(|_| ())
    }

    /// Inform the GAM that the main menu can be activated. This blocks until the message has been delivered
    /// to the GAM.
    ...
    pub fn powerdown_request(&self) -> Result<bool, xous::Error> {
        ...
        if let xous::Result::Scalar1(confirmed) = response {
            if confirmed != 0 { Ok(true) } else { Ok(false) }
        } else {
    ...
    pub fn post_textview(&self, tv: &mut TextView) -> Result<(), xous::Error> {
        ...
        buf.lend_mut(self.conn, Opcode::RenderTextView.to_u32().unwrap())
        ...
        match buf.to_original().unwrap() {
            ...
            _ => panic!(
                "GAM_API: post_textview got a return value from the server that isn't expected or handled"
            ),
        }
    ...
    pub fn bounds_compute_textview(&self, tv: &mut TextView) -> Result<(), xous::Error> {
        ...
        match buf.to_original().unwrap() {
            ...
            _ => panic!(
                "GAM_API: bounds_compute_textview got a return value from the server that isn't expected or handled"
            ),
        }
        tv.set_op(TextOp::Nop);
        Ok(())
    ...
    pub fn draw_line(&self, gid: Gid, line: Line) -> Result<(), xous::Error> {
        let go = GamObject { canvas: gid, obj: GamObjectType::Line(line) };
    ...
    pub fn draw_rectangle(&self, gid: Gid, rect: Rectangle) -> Result<(), xous::Error> {
        let go = GamObject { canvas: gid, obj: GamObjectType::Rect(rect) };
    ...
    pub fn draw_rounded_rectangle(&self, gid: Gid, rr: RoundedRectangle) -> Result<(), xous::Error> {
        let go = GamObject { canvas: gid, obj: GamObjectType::RoundRect(rr) };
        let buf = Buffer::into_buf(go).or(Err(xous::Error::InternalError))?;
        buf.lend(self.conn, Opcode::RenderObject.to_u32().unwrap()).map(|_| ())
    }
    ...
    pub fn draw_bitmap(&self, gid: Gid, bm: &Bitmap) -> Result<(), xous::Error> {
        for (_i, tile) in bm.iter().enumerate() {
            let gt = GamTile { tile: *tile, canvas: gid };
            let buf = Buffer::into_buf(gt).or(Err(xous::Error::InternalError))?;
            buf.lend(self.conn, Opcode::RenderTile.to_u32().unwrap()).map(|_| ())?;
        }
        Ok(())
    }

    pub fn draw_circle(&self, gid: Gid, circ: Circle) -> Result<(), xous::Error> {
        let go = GamObject { canvas: gid, obj: GamObjectType::Circ(circ) };
        let buf = Buffer::into_buf(go).or(Err(xous::Error::InternalError))?;
        buf.lend(self.conn, Opcode::RenderObject.to_u32().unwrap()).map(|_| ())
    }
    ...
    pub fn draw_list(&self, list: GamObjectList) -> Result<(), xous::Error> {
        ...
        buf.lend(self.conn, Opcode::RenderObjectList.to_u32().unwrap()).map(|_| ())
    }
    ...
    pub fn get_canvas_bounds(&self, gid: Gid) -> Result<Point, xous::Error> {
        ...
        let response = send_message(
            ...
            Message::new_blocking_scalar(
                Opcode::GetCanvasBounds.to_usize().unwrap(),
                gid.gid()[0] as _,
                gid.gid()[1] as _,
                gid.gid()[2] as _,
                gid.gid()[3] as _,
    ...
    pub fn set_canvas_bounds_request(&self, req: &mut SetCanvasBoundsRequest) -> Result<(), xous::Error> {
        ...
        buf.lend_mut(self.conn, Opcode::SetCanvasBounds.to_u32().unwrap())
            .or(Err(xous::Error::InternalError))?;
        match buf.to_original().unwrap() {
            ...
            _ => panic!(
                "GAM_API: set_canvas_bounds_request view got a return value from the server that isn't expected or handled"
            ),
        }
        Ok(())
    }
    ...
    pub fn request_content_canvas(&self, token: [u32; 4]) -> Result<Gid, xous::Error> {
        ...
        buf.lend_mut(self.conn, Opcode::RequestContentCanvas.to_u32().unwrap())
            .or(Err(xous::Error::InternalError))?;
        ...
        match buf.to_original().unwrap() {
            ...
            _ => {
                log::error!(
                    "GAM_API: request_content_canvas got a return value from the server that isn't expected or handled"
                );
                Err(xous::Error::InternalError)
            }
        }
    ...
    pub fn type_chars(&self, s: &str) -> Result<bool, xous::Error> {
        ...
        while view.peek().is_some() {
            ...
            send_message(
                self.conn,
                Message::new_scalar(
                    Opcode::KeyboardEvent.to_usize().unwrap(),
    ...
    pub fn set_predictor_api_token(
        &self,
        api_token: [u32; 4],
        gam_token: [u32; 4],
    ) -> Result<(), xous::Error> {
        let at = ApiToken { gam_token, api_token };
    ...
    pub fn set_audio_opcode(&self, opcode: u32, token: [u32; 4]) -> Result<(), xous::Error> {
        let audio_op = SetAudioOpcode { token, opcode };
        let buf = Buffer::into_buf(audio_op).or(Err(xous::Error::InternalError))?;
        buf.lend(self.conn, Opcode::SetAudioOpcode.to_u32().unwrap())
            .or(Err(xous::Error::InternalError))
            .map(|_| ())
    }
    ...
    pub fn set_vibe(&self, enable: bool) -> Result<(), xous::Error> {
        ...
        send_message(self.conn, Message::new_scalar(Opcode::Vibe.to_usize().unwrap(), ena, 0, 0, 0))
            .map(|_| ())
    }
    ...
    pub fn toggle_menu_mode(&self, token: [u32; 4]) -> Result<(), xous::Error> {
        send_message(
            self.conn,
            Message::new_scalar(
                Opcode::ToggleMenuMode.to_usize().unwrap(),
                token[0] as usize,
                token[1] as usize,
                token[2] as usize,
                token[3] as usize,
    ...
    pub fn relinquish_focus(&self) -> Result<(), xous::Error> {
        send_message(
            self.conn,
            Message::new_blocking_scalar(Opcode::RevertFocus.to_usize().unwrap(), 0, 0, 0, 0),
        )
        .map(|_| ())
    }
    ...
    pub fn relinquish_focus_nb(&self) -> Result<(), xous::Error> {
        send_message(self.conn, Message::new_scalar(Opcode::RevertFocusNb.to_usize().unwrap(), 0, 0, 0, 0))
            .map(|_| ())
    }
    ...
    pub fn request_ime_redraw(&self) -> Result<(), xous::Error> {
        send_message(self.conn, Message::new_scalar(Opcode::RedrawIme.to_usize().unwrap(), 0, 0, 0, 0))
            .map(|_| ())
    }
    ...
    pub fn raise_menu(&self, menu_name_str: &str) -> Result<(), xous::Error> {
        ...
        buf.lend_mut(self.conn, Opcode::RaiseMenu.to_u32().unwrap())
            .or(Err(xous::Error::InternalError))
            .expect("couldn't send RaiseMenu opcode");
    ...
    pub fn raise_modal(&self, modal_name: &str) -> Result<(), xous::Error> { self.raise_menu(modal_name) }
    ...
    pub fn bytes_to_bip39(&self, bytes: &Vec<u8>) -> Result<Vec<std::string::String>, xous::Error> {
        ...
        buf.lend_mut(self.conn, Opcode::BytestoBip39.to_u32().unwrap())
            .or(Err(xous::Error::InternalError))
            .expect("couldn't send RaiseMenu opcode");
    ...
    pub fn bip39_to_bytes(&self, bip39: &Vec<std::string::String>) -> Result<Vec<u8>, xous::Error> {
        ...
        if result.data_len == 0 {
            ...
            Ok(result.data[..result.data_len as usize].to_vec())
        }
    }
    ...
    pub fn bip39_suggestions(&self, start: &str) -> Result<Vec<std::string::String>, xous::Error> {
        ...
        let mut buf = Buffer::into_buf(ipc).or(Err(xous::Error::InternalError))?;
        buf.lend_mut(self.conn, Opcode::Bip39Suggestions.to_u32().unwrap())
            .or(Err(xous::Error::InternalError))
            .expect("couldn't send RaiseMenu opcode");
        let result = buf.to_original::<Bip39Ipc, _>().unwrap();
